...
class Loader
{
    ...
    static function run()
    {
        // Modules Part
        $x = 0;
        while(isset(LeafCore::$cfg['modules'][$x])) //Load all Modules
        {
            self::module_load(LeafCore::$cfg['modules'][$x]);
            $x++;
        }
        ...
        // Libs Part
        $x = 0;
        while(isset(LeafCore::$cfg['libs'][$x])) //Load all Libs
        {
            self::lib_load(LeafCore::$cfg['libs'][$x]);
            $x++;
        }
        ...
        while(isset(self::$libs_stack[$x])) // Run each Lib/Class.
        {
            ...
            call_user_func(self::$libs_stack[$x] . '::init');
            $x++;
        }
